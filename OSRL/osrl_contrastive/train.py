...
class ContrastiveCDTTrainConfig(CDTTrainConfig):
    num_buckets: int = 2               
    max_expected_cost: float = 100.0   
    pretrain_steps: int = 0            
    update_steps: int = 70000          
...
def train(args: ContrastiveCDTTrainConfig):
    ...
    # Clean Naming for WandB
    env_short = args.task.split("-")[0].replace("Offline", "")
    args.name = f"{env_short}_{args.num_buckets}B_{args.pretrain_steps}Pre"
    ...
    trainer = ContrastiveCDTTrainer(
        ...
        num_buckets=args.num_buckets,          # ADD THIS
        max_expected_cost=args.max_expected_cost, # ADD THIS
